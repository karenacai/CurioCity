...
export default function Home() {
  ...
  const [_revealedAnswers, setRevealedAnswers] = useState<Set<string>>(new Set())
  ...
  const handleAnswerClick = async (questionId: string, choice: string) => {
    ...
    if (!user) {
      setError('You must be logged in to answer questions')
      return
    }
    ...
    try {
      console.log('Handling answer click:', {
        questionId,
        userId: user.id, // Now safe because we checked above
        choice
      })
      ...
      const result = await saveUserAnswer(questionId, user.id, choice) // Now safe because we checked above
  ...
  return (
    <>
      <PageTitle title="CurioCity | Your Trivia Adventure" description="Generate and answer trivia questions across various categories and difficulty levels" />
      
      <main className="flex min-h-screen flex-col items-center p-24">
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold mb-4">Welcome to your Daily Trivia Generator</h1>
          <p className="text-lg text-gray-600 mb-8">Get started building your trivia knowledge one question at a time!</p>
                ...
                <div className="px-4">
                  <Range
                    ...
                    renderTrack={({ props, children }) => {
                      // Use destructuring with rest to extract key safely
                      // @ts-expect-error - The key property exists at runtime but is not in the type definition
